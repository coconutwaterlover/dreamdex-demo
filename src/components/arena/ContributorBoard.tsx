...
export function ContributorBoard({
  ...
  limit?: number;
  /** Rail version: drop the columns that don't earn their width in a sidebar. */
  compact?: boolean;
  onSettle?: () => void;
  settling?: boolean;
}) {
  ...
  if (!rows.length) {
    return (
      <p className="empty">
        No ballots yet. The first vote of the round opens the board.
      </p>
    );
  }
  return (
      ...
      <table className="board">
        ...
        <tbody>
          {shown.map((row) => {
            ...
            const href = row.tokenId
              ? badgeTokenHref(
                  collection as `0x${string}` | undefined,
                  row.tokenId,
                )
              : null;
            return (
              <tr key={row.wallet} className={isMe ? "is-me" : undefined}>
                ...
                <td>
                  ...
                  {row.pending > 0 && (
                    <span
                      ...
                      title={`${row.pending} ballots waiting on their settlement round`}
                    >
                      {row.pending} pending
                    </span>
                  )}
                ...
                <td
                  className={`right num ${row.points > 0 ? "up" : row.points < 0 ? "down" : ""}`}
                >
                  {signedPoints(row.points)}
                </td>
                <td className="right num dim">
                  {signedPoints(row.seasonPoints)}
                </td>
                <td className="right num">
                  {row.streak > 0 ? `${row.streak}🔥` : "—"}
                  <span className="dim"> / {row.bestStreak}</span>
                </td>
                {!compact && <td className="right num dim">{row.ballotsCast}</td>}
              </tr>
            );
          })}
        </tbody>
          ...
          <tfoot>
            <tr>
              <td colSpan={compact ? 4 : 6}>
                ...
                <span className="foot">
                  {" "}
                  — anyone can settle anyone; your next vote does it for you
                  automatically.
                </span>
              </td>
            </tr>
